...
on(MessageEvents.SEND_MESSAGE_DARK, async (reqObj: DarkMessage, resp: any) => {
	await MessagesService.handleSendMessageDark(reqObj, resp);
	// .then(async () => {
	// 	// A simple solution to listen for messages. Will expand upon this soonTM.
	// 	const funcRef = OnMessageExportMap.get(reqObj.phoneNumber);
	// 	if (funcRef) {
	// 		try {
	// 			await funcRef({data: reqObj.data, source: reqObj.source});
	// 		} catch (e) {
...
onNetPromise<number, void>(MessageEvents.SET_MESSAGE_READ, async (reqObj, resp) => {
	...
	MessagesService.handleSetMessageRead(reqObj, resp).catch((e) =>
		messagesLogger.error(
			`Error occurred in set message read event (${src}), Error: ${e.message}`,
		),
	);
});
