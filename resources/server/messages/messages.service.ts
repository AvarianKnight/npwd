...
import {createGroupHashID, getIdentifiersFromParticipants, messagesLogger} from './messages.utils';
...
class _MessagesService {
	...
	async handleSendMessageDark(reqObj: DarkMessage, resp: PromiseEventResp<void>) {
		try {
			...
			const doesExist = await this.messagesDB.doesConversationExist(conversationList);
			if (!doesExist) {
				conversationId = await MessagesDB.createConversation(
					[authorPhoneNumber, reqObj.phoneNumber],
					conversationList,
					'UNKNOWN NUMBER',
					false,
				);
			} else {
				conversationId = await MessagesDB.getConversationId(conversationList);
			}
			...
			emitNet(MessageEvents.CREATE_MESSAGE_BROADCAST, reqObj.source, {
